...
| 输入   | 衣物图片列表 + 点位信息 + 配饰佩戴描述 |
| 模型   | qwen-image-2.0（通义千问图像模型）    |
| 输出   | 真人模特穿着效果图 URL               |
| 配饰处理 | 生成时如有待生成配饰，弹窗让用户描述佩戴位置 |

### 3.4 保存与记录

- 保存当前搭配（衣物+配饰描述+生成图）到 DIY 记录
- 支持查看历史 DIY 记录
- 支持删除 DIY 记录
...
interface AccessoryItem {
  ...
  position?: { x: number; y: number }; // 位置信息不再必需
}
...
| 衣物融合质量               | 🔴 高 | 使用抠图后的透明背景图；模型选择合适的 task\_type |
